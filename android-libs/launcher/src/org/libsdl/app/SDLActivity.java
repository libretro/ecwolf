...
public class SDLActivity extends Activity implements View.OnSystemUiVisibilityChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Set up JNI
        SDL.setupJNI();
        Log.v(TAG, "setupJNI() complete");

        // Initialize state
        SDL.initialize();
        Log.v(TAG, "initialize() complete");

        // So we can call stuff from static callbacks
        mSingleton = this;
        SDL.setContext(this);
        Log.v(TAG, "context set");

        mClipboardHandler = new SDLClipboardHandler_API11();

        mHIDDeviceManager = HIDDeviceManager.acquire(this);
        Log.v(TAG, "past HIDDeviceManager");
        ...
        Log.v(TAG, "Relative Layout created");
        mLayout.addView(mSurface);
        Log.v(TAG, "surface added");

        // Get our current screen orientation and pass it down.
        mCurrentOrientation = SDLActivity.getCurrentOrientation();
        // Only record current orientation
        SDLActivity.onNativeOrientationChanged(mCurrentOrientation);
        Log.v(TAG, "orientation handler installed");

        setContentView(mLayout);
        Log.v(TAG, "content view set");

        setWindowStyle(false);
        Log.v(TAG, "window style set");

        getWindow().getDecorView().setOnSystemUiVisibilityChangeListener(this);
        Log.v(TAG, "visibility change listener added");
    ...
    public static int getCurrentOrientation() {
        ...
        Log.v(TAG, "got context");
        final Display display = ((WindowManager) context.getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();
        Log.v(TAG, "got display");
