...
void ControlMovement (APlayerPawn *ob)
{
	...
	else
	{
		if(ob->player->ReadyWeapon && ob->player->ReadyWeapon->fovscale > 0)
		{
			...
			// every target. This used to go through xs_ToInt(), whose
			...
			// that conversion returned on a strict-double host bit for bit.
			...
			// The conversion truncated toward zero; a bare >> would floor.
